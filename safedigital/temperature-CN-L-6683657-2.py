...
class TempRiseExperiment(object):
    ...
    def dynamic_t_plot(self, col_name, cur_var_list,time_const,conver_const,**kwargs):
        """General Plotting function of all temp curves
        Args:
        -------
            col_name        - column name to be plotted
            cur_var_list    - list of current variant points
            title           - figure title
            x_label         - x label
            y_label         - y label
        Return:
        ...
        """
        # keywords arguments list
        title = kwargs.get('title', 'Dynamic Temperature Rise')
        x_label = kwargs.get('x_label', 'Time (Hours:Minutes)')
        y_label = kwargs.get('y_label', 'Temperature Rise(K)')
        ...
        # # default line color and styles
        # line_style = kwargs.get('line_style', ['-'] * len(col_name))
        # line_color = kwargs.get('line_color', ['b', 'g', 'r', 'c', 'm', 'y', 'k'] * len(col_name))

        # cut data as long as current variant list
        data_cut = self.data.iloc[:len(cur_var_list),:]
        # Add the current variant column in data_cut
        data_cut['cur_var'] = cur_var_list
        # Add the fitted TR column in data_cut
        fit_tr_list = [data_cut.loc[0,col_name]] + [0] * len(data_cut-1)

        for k in range(1,len(data_cut)):
            fit_tr_list[k] = fit_tr_list[k - 1] * 

        data_cut['fit_tr'] = [              for i in range(len(data_cut))]
        # plot curves as per "col_name_list"
        try:
            for i, name in enumerate(col_name_list):
                # self.interp_data(name)
